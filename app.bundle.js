...
function annualSalaryToMonthlyPayroll(value = 0) {
  return Math.round((Number(value || 0) * 10000) / 12);
}
...
function recommendedCompBudget(cycle = {}, employees = []) {
  ...
  const baseAnnual = relevantEmployees.reduce((acc, item) => acc + Number(item.annualSalary || 0) * 10000, 0);
...
function renderOrgPage(ctx) {
  const { state, visibleOrgs, normalizedOrg, entityChangeHistory, currentSite, visibleEmployees, orgName, employeeName, badge, labelCode } = ctx;
...
function renderPayrollPage(ctx) {
  const {
    ...
    entityChangeHistory, money, badge, byId, employeeName, labelCode, permissions, visibleEmployees
  } = ctx;
...
function renderOnboardingPage(ctx) {
  const { state, visibleOnboarding, orgName, badge, visibleOrgs } = ctx;
  ...
  `;
  return `${summaryCards}${activeSubSection === "checklist" ? checklistView : preboardingView}`;
}
...
function renderMovementPage(ctx) {
  ...
  `;
  return `${summaryCards}${activeSubSection === "candidate" ? candidateView : activeSubSection === "document" ? documentView : actionView}`;
}
...
function renderExitPage(ctx) {
  ...
  `;
  return `${summaryCards}${activeSubSection === "settlement" ? settlementView : activeSubSection === "analysis" ? analysisView : plannedView}`;
}
...
function renderLearningPage(ctx) {
  const { state, visibleCourses, badge, employeeName } = ctx;
  ...
  `;
  return `${summaryCards}${activeSubSection === "completion" ? completionView : courseView}`;
}

function renderReviewPage(ctx) {
  const { state, visibleEvaluationAppeals, visibleCalibrations, byId, employeeName, orgName, badge, visibleEmployees, money } = ctx;
  const appeals = visibleEvaluationAppeals();
  const calibrations = visibleCalibrations();
  const activeEmployees = visibleEmployees().filter((item) => item.status === "재직");
  if (!state.selectedEvaluationCycleId || !state.data.evaluationCycles.some((item) => item.id === state.selectedEvaluationCycleId)) state.selectedEvaluationCycleId = state.data.evaluationCycles[0]?.id || null;
  const selectedCycle = byId(state.data.evaluationCycles, state.selectedEvaluationCycleId) || null;
  const activeSubSection = state.subSection || "cycle";
  const cycleView = `
    <section class="detail-grid">
      <article class="detail-panel">
        <section class="table-card">
          ...
          <table>
            ...
            <tbody>
              ${state.data.evaluationCycles.map((item) => `
        ...
        <section class="detail-section">
          ...
          ${selectedCycle ? `
            <form class="detail-form" id="reviewCycleForm">
              ...
              <label>평가명<input type="text" name="title" value="${selectedCycle.title}" required /></label>
              <label>대상 기간<input type="text" name="period" value="${selectedCycle.period}" required /></label>
              <label>대상 인원<input type="number" name="targetCount" value="${selectedCycle.targetCount}" required /></label>
              <label>진행률<input type="number" name="completionRate" value="${selectedCycle.completionRate || 0}" required /></label>
              <label>권장 인상률(%)<input type="number" name="recommendedIncreaseRate" value="${Number(selectedCycle.recommendedIncreaseRate || recommendedCompRate(selectedCycle))}" required /></label>
              <label>상태<select name="status">${["planning","ongoing","completed"].map((item) => `<option value="${item}" ${selectedCycle.status === item ? "selected" : ""}>${labelCode(item)}</option>`).join("")}</select></label>
              <div class="full actions">
                <button type="submit" class="button">저장</button>
                <button type="button" class="button-secondary" data-review-action="create-comp" data-id="${selectedCycle.id}" ${state.data.compensationPlans.some((plan) => plan.cycleId === selectedCycle.id) ? "disabled" : ""}>보상안 생성</button>
              </div>
            </form>
  ...
  `;
  return activeSubSection === "appeal" ? appealView : activeSubSection === "calibration" ? calibrationView : cycleView;
}

function renderCompensationPage(ctx) {
  const { state, byId, money, badge, visibleEmployees } = ctx;
  const activeEmployees = visibleEmployees().filter((item) => item.status === "재직");
  ...
  const activeSubSection = state.subSection || "plan";
  const planView = `
    <section class="detail-grid">
      <article class="detail-panel">
        <section class="table-card">
  ...
  const promotionView = `
    <section class="grid-3">
      ${state.data.compensationPlans.map((item) => `
        <article class="stat-card">
          <h3>${byId(state.data.evaluationCycles, item.cycleId)?.title || "-"}</h3>
          <strong>${item.targetCount}명</strong>
          <span>인상률 ${Number(item.avgIncreaseRate || 0)}% / 예산 ${money(item.budget)}원</span>
        </article>
      `).join("") || `<div class="empty">보상안이 없습니다. 현재 재직자 ${activeEmployees.length}명 기준으로 평가 완료 사이클에서 생성하세요.</div>`}
    </section>
  `;
  return activeSubSection === "promotion" ? promotionView : planView;
}
...
function sectionBindEvents(ctx) {
  ...
  const recalculatePayrollPeriod = (period) => {
    ...
    const totalAnnualSalary = Math.max(1, activeEmployees.reduce((acc, item) => acc + Number(item.annualSalary || 0), 0));
    ...
    activeEmployees.forEach((employee) => {
      ...
      const bonusPay = linkedPlan ? Math.round((Number(employee.annualSalary || 0) / totalAnnualSalary) * Number(linkedPlan.usedBudget || 0)) : 0;
  ...
  if (state.section === "people") {
    ...
    el("employeeDetailForm")?.addEventListener("submit", (event) => {
      ...
      if (!employee) return;
      const residentValidation = validateResidentNumber(formData.residentNumber);
        ...
        sections: [
          ...
          },
          {
            ...
            rows: [
              ...
              { label: "연봉", value: permissions.canViewSalary(employee.id) ? `${money(employee.annualSalary || 0)}만원` : "비공개" }
            ]
          },
          {
            title: "인사 확장 정보",
            ...
            <div class="modal-grid">
              ${fields.map((field) => {
                ...
                if (field.type === "select") {
                  return `<label class="${cls}">${field.label}<select name="${field.name}" ${field.required ? "required" : ""}>${options}</select></label>`;
                }
                if (field.type === "textarea") {
                  return `<label class="${cls}">${field.label}<textarea name="${field.name}" ${field.required ? "required" : ""}>${field.value || ""}</textarea></label>`;
                }
                return `<label class="${cls}">${field.label}<input type="${field.type || "text"}" name="${field.name}" value="${field.value || ""}" placeholder="${field.placeholder || ""}" ${field.required ? "required" : ""} /></label>`;
              }).join("")}
            </div>
            <footer>
  ...
  function openSectionForm(section) {
    ...
    if (section === "people") {
      openModal("직원 등록", [
        ...
        { name: "annualSalary", label: "연봉", type: "number", required: true }
      ], (data) => {
    ...
    if (section === "compensation") {
      ...
      openModal("보상안 시뮬레이션", [
        ...
        { name: "targetCount", label: "대상 인원", type: "number", required: true, value: String(defaultCycle.targetCount || activeEmployees.length || 0) },
        { name: "avgIncreaseRate", label: "평균 인상률(%)", type: "number", required: true, value: String(recommendedCompRate(defaultCycle)) },
        { name: "budget", label: "총 예산", type: "number", required: true, value: String(recommendedCompBudget(defaultCycle, activeEmployees)) }
      ], (data) => {
        ...
        state.data.compensationPlans.unshift({
          ...
          usedBudget: Math.round(Number(data.budget) * 0.9),
          status: "simulated"
        });
...
function createHrSelectors(ctx) {
  ...
  function currentUser() {
    const matched = byId(state.data.employees, state.currentUserId) || state.data.employees[0];
...
/* src/app/app.js */
      ...
      const SUBSECTION_ITEMS = {
        ...
        learning: [
          ...
          { id: "completion", label: "이수점검" }
        ],
        review: [
          { id: "cycle", label: "평가사이클" },
      ...
      const SUBSECTION_META = {
        ...
        learning: {
          ...
          completion: { title: "이수점검", description: "과정별 이수율과 미이수 위험 과정을 점검합니다.", action: "교육 계획 등록" }
        },
        review: {
          cycle: { title: "평가사이클", description: "평가 주기와 완료율을 관리합니다.", action: "사이클 생성" },
        ...
        compensation: {
          ...
          promotion: { title: "승진후보", description: "승진 후보 현황과 추천 상태를 점검합니다.", action: "보상안 생성" }
        },
        payroll: {
      ...
      async function loadData() {
        ...
        try {
          ...
          state.data = snapshot.exists() ? normalizeData(snapshot.val()) : createSeedData(SCHEMA_VERSION);
          const matchedEmployee = (state.data.employees || []).find((item) => item.email && item.email === state.authUser.email);
          state.currentUserId = matchedEmployee?.id || state.currentUserId;
        } catch (error) {
      ...
      function renderSection() {
        ...
        switch (state.section) {
          ...
          case "onboarding": return renderOnboardingPage({
            state, visibleOnboarding, orgName, badge, visibleOrgs
          });
          ...
          case "org": return renderOrgPage({
            ...
            visibleEmployees, orgName, employeeName, badge, labelCode
          });
          ...
          case "learning": return renderLearningPage({
            state, visibleCourses, badge, employeeName
          });
          ...
          case "compensation": return renderCompensationPage({
            state, byId, money, badge, visibleEmployees
          });
          case "payroll": return renderPayrollPage({
            state, visiblePayrollAnomalies, normalizedPayrollPeriod, normalizedPayrollRule,
            payrollAuditLogs, entityChangeHistory, money, badge, byId, employeeName, labelCode, permissions, visibleEmployees
          });
